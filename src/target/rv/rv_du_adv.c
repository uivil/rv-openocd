...
static int rv_adv_jtag_init(struct rv_jtag *jtag_info)
{
	struct rv_tap_ip *tap_ip = jtag_info->tap_ip;
	...
	jtag_info->rv_jtag_inited = 1;

	/* TAP reset - not sure what state debug module chain is in now */
	jtag_info->rv_jtag_module_selected = DC_NONE;
	...
	if (rv_du_adv.options & ADBG_USE_HISPEED)
		LOG_INFO("adv debug unit is configured with option ADBG_USE_HISPEED");

	if (rv_du_adv.options & ENABLE_JSP_SERVER) {
		if (rv_du_adv.options & ENABLE_JSP_MULTI)
 ...
 */
static int adbg_select_module(struct rv_jtag *jtag_info, int chain)
{
	if (jtag_info->rv_jtag_module_selected == chain)
		return ERROR_OK;
	...
	jtag_info->rv_jtag_module_selected = chain;
 ...
 * n bits index
 */
static int adbg_select_ctrl_reg(struct rv_jtag *jtag_info, uint8_t regidx)
{
	int index_len;
	uint32_t opcode;
	uint32_t opcode_len;
	...
	if (jtag_info->current_reg_idx[jtag_info->rv_jtag_module_selected] == regidx)
		return ERROR_OK;

	switch (jtag_info->rv_jtag_module_selected) {
	case DC_WISHBONE:
	...
	default:
		LOG_ERROR("Illegal debug chain selected (%i) while selecting control register",
			  jtag_info->rv_jtag_module_selected);
		return ERROR_FAIL;
	}
	...
	jtag_info->current_reg_idx[jtag_info->rv_jtag_module_selected] = regidx;

	return ERROR_OK;
}

/* Write control register (internal to the debug unit) */
static int adbg_ctrl_write(struct rv_jtag *jtag_info, uint8_t regidx,
...
static int adbg_ctrl_read(struct rv_jtag *jtag_info, uint32_t regidx,
 ...
 */
static int adbg_burst_command(struct rv_jtag *jtag_info, uint32_t opcode,
...
static int adbg_wb_burst_read(struct rv_jtag *jtag_info, int size,
...
retry_read_full:
	...
	if (jtag_info->rv_jtag_module_selected == DC_WISHBONE &&
	    !(rv_du_adv.options & ADBG_USE_HISPEED)) {
...
static int adbg_wb_burst_write(struct rv_jtag *jtag_info, const uint8_t *data, int size,
...
static int rv_adv_jtag_read_cpu(struct rv_jtag *jtag_info,
		uint32_t addr, int count, uint32_t *value)
{
	int retval;
	if (!jtag_info->rv_jtag_inited) {
		retval = rv_adv_jtag_init(jtag_info);
		if (retval != ERROR_OK)
			return retval;
	}
...
static int rv_adv_jtag_write_cpu(struct rv_jtag *jtag_info,
...
static int rv_adv_cpu_stall(struct rv_jtag *jtag_info, int action)
{
	int retval;
	if (!jtag_info->rv_jtag_inited) {
		retval = rv_adv_jtag_init(jtag_info);
...
static int rv_adv_is_cpu_running(struct rv_jtag *jtag_info, int *running)
{
	int retval;
	if (!jtag_info->rv_jtag_inited) {
		retval = rv_adv_jtag_init(jtag_info);
	...
	int current = jtag_info->rv_jtag_module_selected;
...
static int rv_adv_cpu_reset(struct rv_jtag *jtag_info, int action)
{
	int retval;
	if (!jtag_info->rv_jtag_inited) {
		retval = rv_adv_jtag_init(jtag_info);
...
static int rv_adv_jtag_read_memory(struct rv_jtag *jtag_info,
...
static int rv_adv_jtag_write_memory(struct rv_jtag *jtag_info,
...
static struct rv_du rv_du_adv = {
	.name                     = "adv",
	.options                  = ADBG_USE_HISPEED | ENABLE_JSP_SERVER ,
	.rv_jtag_init           = rv_adv_jtag_init,

	.rv_is_cpu_running      = rv_adv_is_cpu_running,
	.rv_cpu_stall           = rv_adv_cpu_stall,
	.rv_cpu_reset           = rv_adv_cpu_reset,

	.rv_jtag_read_cpu       = rv_adv_jtag_read_cpu,
	.rv_jtag_write_cpu      = rv_adv_jtag_write_cpu,

	.rv_jtag_read_memory    = rv_adv_jtag_read_memory,
	.rv_jtag_write_memory   = rv_adv_jtag_write_memory
};

int rv_du_adv_register(void)
{
	list_add_tail(&rv_du_adv.list, &du_list);
	return 0;
}
